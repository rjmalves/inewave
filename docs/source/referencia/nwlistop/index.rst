.. _nwlistop:

NWLISTOP
===========

O módulo ``inewave.nwlistop`` contém as classes para leitura dos arquivos
de saída do pós-processador NWLISTOP. Cada classe representa um arquivo de
resultado específico, fornecendo acesso aos dados de operação do sistema
elétrico gerados pelo modelo NEWAVE.

.. currentmodule:: inewave.nwlistop

.. autosummary::
   :nosignatures:

   Cbomb
   Cbombsin
   Cdef
   Cdefsin
   CustoFuturo
   Cmarg
   Cmargmed
   Coper
   Corteolm
   Cterm
   Ctermsin
   CviolEletrica
   CviolEletricasin
   CviolRhq
   CviolRhqsin
   CviolRhv
   CviolRhvsin
   Def
   Defsin
   Eaf
   Eafb
   Eafbm
   Eafbsin
   Eafm
   Earmf
   Earmfm
   Earmfp
   Earmfpm
   Earmfpsin
   Earmfsin
   Edesvc
   Edesvcm
   Edesvcsin
   Evapo
   Evapom
   Evaporsin
   Evert
   Evertm
   Evertsin
   Exces
   Excessin
   FormRHQ
   FormRHV
   Fteolm
   Fteolsin
   Geol
   Geolm
   Geolsin
   Ghidr
   Ghidrm
   Ghidrsin
   Ghiduh
   Ghmax
   GhmaxFpha
   GhmaxFphc
   Ghmaxm
   Ghmaxmr
   Ghmaxr
   Ghmaxrsin
   Ghmaxsin
   Ghtot
   Ghtotm
   Ghtotsin
   Gtert
   Gttot
   Gttotsin
   Hjus
   Hliq
   Hmont
   Intercambio
   Invade
   Invadem
   Mediasmerc
   Mediasree
   Mediasrep
   Mediasrhq
   Mediasrhv
   Mediassin
   Mediasusie
   Mediasusih
   Mediasusit
   Mercl
   Merclsin
   Mevmin
   Mevminm
   Mevminsin
   Nwlistopdat
   Perdf
   Perdfm
   Perdfsin
   Pivarm
   Pivarmincr
   Qafluh
   Qbomb
   Qdesviouh
   Qincruh
   Qturuh
   Qvertuh
   Rhslppdf
   Rhslpptb
   ValorAgua
   Varmpuh
   Varmuh
   Vento
   Vevapuh
   ViolEletrica
   ViolEletricasin
   ViolEvmin
   ViolEvminm
   ViolEvminsin
   ViolFpha
   ViolGhmin
   ViolGhminm
   ViolGhminsin
   ViolGhminuh
   ViolLppDfmax
   ViolLppDfmaxm
   ViolLppDfmaxsin
   ViolLppTbmax
   ViolLppTbmaxm
   ViolLppTbmaxsin
   ViolNegEvap
   ViolNegVretiruh
   ViolPosEvap
   ViolPosVretiruh
   ViolRHQ
   ViolRHV
   ViolTurbmax
   ViolTurbmin
   ViolVazmax
   ViolVazmin
   Vmort
   Vmortm
   Vmortsin
   Vretiradauh
   Verturb
   Verturbm
   Verturbsin

.. toctree::
   :maxdepth: 2

   arquivos/nwlistopdat
   arquivos/cviol_rhq_sin
   arquivos/cviol_rhq
   arquivos/cviol_rhv_sin
   arquivos/cviol_rhv
   arquivos/cbomb
   arquivos/cbombsin
   arquivos/cdef
   arquivos/cdefsin
   arquivos/cviol_eletrica
   arquivos/cviol_eletricasin
   arquivos/cmarg
   arquivos/cmargmed
   arquivos/coper
   arquivos/custo_futuro
   arquivos/corteolm
   arquivos/cterm
   arquivos/ctermsin
   arquivos/deficit
   arquivos/defsin
   arquivos/viol_eletrica
   arquivos/viol_eletricasin
   arquivos/viol_vazmin
   arquivos/vretiradauh
   arquivos/viol_fpha
   arquivos/viol_lpp_tbmax
   arquivos/viol_lpp_tbmaxm
   arquivos/viol_lpp_tbmaxsin
   arquivos/viol_lpp_dfmax
   arquivos/viol_lpp_dfmaxm
   arquivos/viol_lpp_dfmaxsin
   arquivos/viol_turbmax
   arquivos/viol_turbmin
   arquivos/viol_vazmax
   arquivos/viol_pos_evap
   arquivos/viol_neg_evap
   arquivos/viol_pos_vretiruh
   arquivos/viol_neg_vretiruh
   arquivos/eaf
   arquivos/eafm
   arquivos/eafb
   arquivos/eafbm
   arquivos/eafbsin
   arquivos/earmf
   arquivos/earmfm
   arquivos/earmfsin
   arquivos/earmfp
   arquivos/earmfpm
   arquivos/earmfpsin
   arquivos/edesvc
   arquivos/edesvcm
   arquivos/edesvcsin
   arquivos/evapo
   arquivos/evapom
   arquivos/evaporsin
   arquivos/evert
   arquivos/evertm
   arquivos/evertsin
   arquivos/exces
   arquivos/excessin
   arquivos/form_rhq
   arquivos/form_rhv
   arquivos/fteolm
   arquivos/fteolsin
   arquivos/geol
   arquivos/geolm
   arquivos/geolsin
   arquivos/ghidr
   arquivos/ghidrm
   arquivos/ghidrsin
   arquivos/ghiduh
   arquivos/ghmax_fpha
   arquivos/ghmax_fphc
   arquivos/gh_fphexat
   arquivos/ghmax
   arquivos/ghmaxr
   arquivos/ghmaxm
   arquivos/ghmaxmr
   arquivos/ghmaxsin
   arquivos/ghmaxrsin
   arquivos/ghtot
   arquivos/ghtotm
   arquivos/ghtotsin
   arquivos/gtert
   arquivos/gttot
   arquivos/gttotsin
   arquivos/hjus
   arquivos/hliq
   arquivos/hmont
   arquivos/intercambio
   arquivos/invade
   arquivos/invadem
   arquivos/mediassin
   arquivos/mediasmerc
   arquivos/mediasree
   arquivos/mediasusih
   arquivos/mediasusit
   arquivos/mediasusie
   arquivos/mediasrep
   arquivos/mediasrhq
   arquivos/mediasrhv
   arquivos/mercl
   arquivos/merclsin
   arquivos/mevmin
   arquivos/mevminm
   arquivos/mevminsin
   arquivos/perdf
   arquivos/perdfm
   arquivos/perdfsin
   arquivos/pivarm
   arquivos/pivarmincr
   arquivos/qafluh
   arquivos/qincruh
   arquivos/rhslppdf
   arquivos/rhslpptb
   arquivos/valor_agua
   arquivos/varmuh
   arquivos/varmpuh
   arquivos/qbomb
   arquivos/qdesviouh
   arquivos/vento
   arquivos/qvertuh
   arquivos/verturb
   arquivos/verturbm
   arquivos/verturbsin
   arquivos/viol_evmin
   arquivos/viol_evminm
   arquivos/viol_evminsin
   arquivos/viol_ghmin
   arquivos/viol_ghminm
   arquivos/viol_ghminsin
   arquivos/viol_ghminuh
   arquivos/viol_rhq
   arquivos/viol_rhv
   arquivos/vmort
   arquivos/vmortm
   arquivos/vmortsin
   arquivos/qturuh
   arquivos/vevapuh
