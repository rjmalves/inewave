from inewave.config import MAX_PATAMARES, MESES_DF, MAX_SERIES_SINTETICAS
...
class PEE(Block):
    """
    Bloco com a informaçao do PEE associado aos valores
    de geraçao.
    """

    BEGIN_PATTERN = r"GERACAO EOLICA \(MWmes\) "
    END_PATTERN = ""

    def __init__(self, previous=None, next=None, data=None) -> None:
        super().__init__(previous, next, data)
        self.__linha = Line([LiteralField(40, 66)])

    def __eq__(self, o: object) -> bool:
        if not isinstance(o, PEE):
            return False
        bloco: PEE = o
        if not all(
            [
