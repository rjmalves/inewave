...
class BlocoCustoMarginalMedio(Bloco):
    """
    Bloco com as informações das tabelas de energias armazenada final
    por série e por mês/ano de estudo.
    """

    str_inicio = "CUSTO MARGINAL DE DEMANDA - MEDIA PATAMARES"
    str_fim = ""

    def __init__(self):

        super().__init__(BlocoCustoMarginalMedio.str_inicio, "", True)

        self._dados = ["", pd.DataFrame()]

    def __eq__(self, o: object):
        if not isinstance(o, BlocoCustoMarginalMedio):
            return False
        bloco: BlocoCustoMarginalMedio = o
        return all(
            [
...
class LeituraCmarg00med(LeituraBlocos):
    """
    Realiza a leitura dos arquivos cmarg00x-med.out
    existentes em um diretório de saídas do NEWAVE.

    Esta classe contém o conjunto de utilidades para ler
    e interpretar os campos de arquivos cmarg00x-med.out, construindo
    objetos `Cmarg00med` cujas informações são as mesmas dos arquivos.
    ...
    def _cria_blocos_leitura(self) -> List[Bloco]:
        return [BlocoCustoMarginalMedio()]
