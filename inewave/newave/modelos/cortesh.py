...
class SecaoDadosCortesH(Section):
    ...
    @property
    def ordens_modelos_parp_uhes(self) -> pd.DataFrame:
        offset = self.__offset_decimo_terceiro_registro()
        tamanho_bloco_dados_uhes = int(
            self.__tamanho_decimo_quarto_registro / self.numero_maximo_uhes
        )
        dados = np.array(
            self.data[offset : offset + self.__tamanho_decimo_quarto_registro]
        ).reshape((-1, tamanho_bloco_dados_uhes))
        cols_dados = [
            "codigo_usina",
            "indice_usina",
        ...
        cols_ordens = (
            [
                f"ordem_modelo_parp_estagio_pre_{i}"
                for i in range(1, self.numero_estagios_pre + 1)
            ]
            + [
                f"ordem_modelo_parp_estagio_{i}"
                for i in range(1, self.numero_estagios_estudo + 1)
            ]
            + [
                f"ordem_modelo_parp_estagio_pos_{i}"
                for i in range(1, self.numero_estagios_pos + 1)
            ]
        )
        df = pd.DataFrame(
            dados,
            columns=cols_dados + cols_ordens,
        )
        df["codigo_interno_ree"] = df.apply(
            lambda linha: self.codigo_interno_ree_uhes[
                self.codigos_uhes.index(linha["codigo_usina"])
        ...
        return df[cols_dados + ["codigo_interno_ree"] + cols_ordens]
