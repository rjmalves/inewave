...
## Summary (through epic-06-testing-improvements)
...
- **`-> Any` on `__getattr__` in lazy import `__init__.py`**: Both `inewave/newave/__init__.py` and `inewave/nwlistop/__init__.py` require this annotation under strict mode.
- **`_DATA_BLOCK_TYPES` tuple for archive configuration**: `_ArquivoSerieBase` and `_ArquivoSeriePatamarBase` expose `_DATA_BLOCK_TYPES: tuple[type, ...]` as a class variable. Subclasses override it to change isinstance dispatch.
...
- **`# type: ignore[import-untyped]` for all pandas/numpy imports**: 138 instances. Pandas 2.2.3 and numpy 2.2.1 do NOT provide stubs recognized by mypy.
...
- **tempfile round-trip for parametrized binary readers**: When `read()` requires external dimensions, use `tempfile.NamedTemporaryFile(delete=False)` + `os.unlink(tmp)` in a try/finally block. Cannot use mock_open for write-then-re-read when dimensions must be passed. See `tests/newave/test_engnat.py` lines 72-91.
- **NOTE comment as skip-documentation for impossible tests**: Files where round-trip tests are architecturally impossible end with `# NOTE: {reason}, round-trip test not applicable`. Three canonical reasons: output-only file, write() not implemented, binary parametrized read. See `tests/newave/test_energiab.py`, `test_eco_fpha.py`, `test_pmo.py`.
- **Import aliasing to resolve model-class name collisions in tests**: When two handlers share identically-named model classes, use `as`-aliases: `from inewave.nwlistop.modelos.cmargmed import CmargsAnos28 as CmargmedAnos28`. See `tests/nwlistop/test_version_validate.py` lines 25-32.
...
- **`warn_return_any = false` globally and per-module**: cfinterface's `Section.data` and `Block.data` are `Any`-typed. All strict overrides in `pyproject.toml` include `warn_return_any = false`.
...
- **nwlistop handlers need no file-level round-trip tests**: Block-level coverage in `tests/nwlistop/test_tabela_serie_anual.py` and `test_tabela_serie_patamar_anual.py` is sufficient. Adding 160 `@pytest.mark.skip` stubs was tried and reverted as dead code.
- **`-n auto` NOT set as pytest default**: Parallel execution is opt-in. Default sequential run is safer for debugging.
- **Permanently-skipped tests are dead code**: If a test can never pass due to architecture (write() raises NotImplementedError), a NOTE comment is the correct documentation mechanism. `@pytest.mark.skip` implies fixability.
...
- `tests/conftest.py`: Empty placeholder — root conftest for future session-scoped fixtures.

### Conventions

- **Import ordering**: `cfinterface` imports first, then `inewave` local imports, then `typing` last.
- **No DeprecationWarning suppression in production code**: Warnings eliminated at source; test-only filter in `pyproject.toml`.
...
- **DeprecationWarning filter scope**: `"ignore::DeprecationWarning:inewave.nwlistop"` — module-scoped, not global.
- **Test baseline is 1140 tests**: As of epic-06 completion (1134 + 2 newave round-trip + 4 validation).
...
- **Audit write() before scoping round-trip tests**: 93% of planned newave round-trip targets had `Block.write()` raising `NotImplementedError`. Grep for `NotImplementedError` in block write methods before scoping any round-trip testing epic.
- **`@pytest.mark.skip` implies a fixable test**: Use NOTE comments for tests that are architecturally impossible. Skip decorators imply the test could eventually pass; they mislead future developers.
- **nwlistop DeprecationWarning count depends on which handlers are exercised**: With lazy loading, only handlers reached by tests emit warnings. Projected 108 warnings; actual 32. Warning counts in projections should be treated as upper bounds.
