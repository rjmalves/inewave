...
def converte_arq_teste_para_f64() -> bytes:
    h = Hidr.read(bytes_gz(ARQ_TESTE))
...
def test_registro_uhe_hidr_f64():
    ...
    r.read(fp_gz(ARQ_TESTE_F64), storage="BINARY")
...
def test_atributos_encontrados_hidr_f64():
    h = Hidr.read(bytes_gz(ARQ_TESTE_F64))
    assert h.cadastro is not None
...
def test_eq_hidr():
    h1 = Hidr.read(bytes_gz(ARQ_TESTE))
    h2 = Hidr.read(bytes_gz(ARQ_TESTE))
    assert h1 == h2


def test_eq_hidr_f64():
    h1 = Hidr.read(bytes_gz(ARQ_TESTE_F64))
    h2 = Hidr.read(bytes_gz(ARQ_TESTE_F64))
    assert h1 == h2
...
def test_deteccao_formato_f32_600():
    ...
    registro = fp_gz(ARQ_TESTE).read(RegistroUHEHidr.TAMANHO_REGISTRO)
...
def test_deteccao_formato_f64_600():
    ...
    registro = fp_gz(ARQ_TESTE_F64).read(RegistroUHEHidrF64.TAMANHO_REGISTRO)
    conteudo = registro * 600
    h = Hidr.read(conteudo)
    assert h.tamanho_registro == RegistroUHEHidrF64.TAMANHO_REGISTRO
    assert len(h.cadastro) == 600
...
def test_conversao_para_f64():
    ...
    h32 = Hidr.read(bytes_gz(ARQ_TESTE))
...
def test_conversao_para_f32():
    h64 = Hidr.read(bytes_gz(ARQ_TESTE_F64))
    h32 = Hidr.read(bytes_gz(ARQ_TESTE_F64))
...
def test_deteccao_formato_desconhecido():
    ...
    conteudo = fp_gz(ARQ_TESTE).read(82368)
...
def test_leitura_escrita_hidr():
    ...
    h2 = Hidr.read(b"".join(linhas_escritas))
    assert h1 == h2
